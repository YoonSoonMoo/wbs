    ...
    const STEPS = [
        {
            ...
            text:
                ...
                '<li><b>이력</b> — 세부항목·담당자·일자·공수·진행률의 변경 이력을 조회합니다 (이력 기록이 켜진 프로젝트만).</li>' +
                '<li><b>데이터관리</b> — CSV/Excel 내보내기, Excel 붙여넣기, 붙여넣기용 샘플 파일 다운로드가 이 메뉴에 모여 있습니다.</li>' +
                '<li><b>사용자 · 로그아웃</b> — 구분선 오른쪽은 내 계정 영역입니다.</li>' +
                '</ul>' +
                '화면 기능 가이드(이 튜토리얼)는 좌측 상단 제목 옆 <b>?</b> 버튼으로 언제든 다시 볼 수 있습니다.',
            placement: 'bottom',
        },
        {
        ...
        },
        {
            ...
            title: '그리드 편집',
            text: '엑셀처럼 씁니다. 클릭은 셀 선택, 편집은 더블클릭(또는 Enter·바로 입력). 드래그·Shift로 범위를 잡아 Delete로 값 삭제, Ctrl+C로 복사해 엑셀에 붙여넣고, Ctrl+V로 엑셀 값을 그대로 채울 수 있습니다. 세부항목/진행상태는 더블클릭 팝업으로 개행까지 입력, 우클릭으로 행 삽입·복제·삭제 메뉴가 열립니다.',
            placement: 'bottom',
            maxHeight: 200,
        },
    ];
